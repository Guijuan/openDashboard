...
router.post('/getDataInfo', function(req, res, next){
    ...
    let dataDimensions = hDataBuffer.getAllDimensions()
    let resdata = []
    ...
    dataDimensions.forEach(function(d,i){
        ...
        let dataCounts = hDataBuffer.getDataLength(name)
            pagesCounts = hDataBuffer.getDataPagesCount(name)
            tempdimensions = []
...
router.post('/getSingleData' ,function(req, res, next){
    ...
    dimensions = hDataBuffer.getSingleDimensions(params.dataName)
    if(params.preview == 0){
        //数据全部返回
        values = hDataBuffer.getSingleData(params.dataName)
    } else {
        //返回某一页面
        values = hDataBuffer.getPageData(params.dataName, params.page)
    }
...
router.post('/getDatalist', function(req, res, next){
    res.setHeader('Content-Type', 'application/json');
    res.json(hDataBuffer.getAllDimensions())
})
...
router.post('/getData' ,function(req, res, next){
    ...
    let values = hDataBuffer.getData(dataName)
    let dimensions = hDataBuffer.getSingleDimensions(dataName)
...
router.post('/getDataDetail' ,function(req, res, next){
    ...
    resData = {
        "dimensions": hDataBuffer.getDataDimensions(dataName),
        "description": dataName,
        "data": {
            "values": hDataBuffer.getData(dataName),
        },
        "title": {
            "text": "",
            "anchor": "",
...
router.post("/testdata", function(req, res, next){
    ...
    res.setHeader("Content-Type", "application/json");
    //console.log(dataProcess.getDataFromDB())
    res.json(hDataBuffer.data[dataName])
})
